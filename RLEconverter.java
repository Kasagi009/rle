...
public String compressLine(String line, char[] fileChars){
   ...
   for( int i = 0; i < line.length(); i++) {
     ...
     if( current == prev ){
       ...
       if( compressedLine.equals("") ){
         compressedLine = compressedLine.concat( counter.toString() );
         counter = 1;
       }
       else{
       compressedLine = compressedLine.concat("," + counter.toString());
  ...
  public String[] compressLines(String[] lines, int dataSize, char[] fileChars){
      //TODO: Implement this method
      String[] imageToText = new String[dataSize];
      for( int i = 0; i <dataSize; i++){
        imageToText[i] = compressLine(lines[i], fileChars);
      }
      return imageToText;
}
...
public String getCompressedFileStr(String[] compressed, char[] fileChars) {
    ...
    compressedFile = compressedFile.concat(fileChars[0] + "," + fileChars[1]);
    for( int i = 0; i < compressed.length; i++){
      compressedFile = compressedFile.concat("/n" + compressed[i]);
    }
   ...
   */
   public String decompressLine(String line, char[] fileChars){
      ...
      for( int i = 0; i < lineArray.length; i++){
        counter = Integer.valueOf(lineArray[i]);
        for( int j = 0; j < counter; j++){
          if( flip == false){
            decompressedLine = decompressedLine.concat(Character.toString(fileChars[0]));
          }
          else{
            decompressedLine = decompressedLine.concat(Character.toString(fileChars[1]));
          }
        }
        flip = !flip;
      }
  ...
  public String[] decompressLines(String[] lines, int dataSize){
     //TODO: Implement this method
     String[] textToImage = new String[dataSize];
     char[] charFile = new char[2];
     charFile[0] = lines[0].charAt(0);
     charFile[1] = lines[0].charAt(2);
     for( int i = 1; i <lines.length; i++){
       textToImage[i-1] = decompressLine(lines[i], charFile);
     }
     return textToImage;
  }
  ...
  public String getDecompressedFileStr(String[] decompressed){
     String data = "";
   //TODO: Implement this method
   data = data.concat(decompressed[0].charAt(0) + "," + decompressed[0].charAt(2));
   for( int i = 0; i < decompressed.length; i++){
     data = data.concat("/n" + decompressed[i]);
   }
   return data;
  }
...
//TODO: Implement this method
  ...
  charList[0] = decompressed[0].charAt(0);
  for( String line : decompressed){
    for( int i = 0; i < line.length(); i++){
      if( line.charAt(i) != charList[0]){
        charList[1] = line.charAt(i);
        return charList;
      }
      else{
        continue;
